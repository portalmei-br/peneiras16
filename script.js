...
let searchLocationData = null; // NOVA VARIÁVEL para armazenar dados da busca
...
async function handleSearch() {
    ...
    try {
        ...
        // ARMAZENAR DADOS DA LOCALIZAÇÃO DA BUSCA
        searchLocationData = {
            cidade: data.localidade,
            estado: data.uf,
            bairro: data.bairro,
            cep: cep,
            endereco_completo: `${data.localidade}, ${data.uf}`
        };
...
// FUNÇÃO MODIFICADA PARA CRIAR CARD DE RESULTADO - INCLUI CIDADE/ESTADO DA BUSCA
function createResultCard(peneira) {
    ...
    // CRIAR ENDEREÇO MODIFICADO COM CIDADE/ESTADO DA BUSCA
    let enderecoExibido = peneira.endereco;
    if (searchLocationData) {
        // Substituir a cidade/estado do endereço original pela cidade/estado da busca
        const enderecoPartes = peneira.endereco.split(', ');
        if (enderecoPartes.length >= 2) {
            // Manter o local específico (ex: "Vila Belmiro") e substituir cidade/estado
            const localEspecifico = enderecoPartes[0];
            enderecoExibido = `${localEspecifico}, ${searchLocationData.cidade}, ${searchLocationData.estado}`;
        } else {
            // Se não houver local específico, usar apenas cidade/estado da busca
            enderecoExibido = `${searchLocationData.cidade}, ${searchLocationData.estado}`;
        }
    }
